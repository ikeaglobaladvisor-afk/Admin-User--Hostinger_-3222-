...
var totaluser = firebase.database().ref().child("bonus_order");
var alluser = firebase.database().ref().child("bonus_order");
...
function stats(key) {
  ...
  var ref51 = firebase.database().ref().child("bonus_order").child(key);
  ...
  ref51.on("value", function (snapshot4) {
    if (snapshot4.exists()) {
      ...
      bnumber = parseInt(snapshot4.val().bnumber);
...
function validate341() {
  ...
  var noti = document.getElementById("camount22").value;
  var bnumber2 = document.getElementById("camount23").value;


  if (noti == "") {
    alert("Bonus number must be filled out");
    return false;
  }

   if (bnumber2 == "") {
    alert("Task number must be filled out");
    return false;
  }
  ...
  var ref9 = firebase.database().ref().child("bonus_order");
  ...
  if (didConfirm == true) {
    ...
    ref9.child(ke2).set({
      ...
      window.location = "bonus_order.html";
    });
